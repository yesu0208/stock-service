...
class PostServiceTest {
    ...
    @Mock private DislikeRepository dislikeRepository;
    @Mock private LikeDislikeService likeDislikeService;
    ...
    void givenUnchangeableIdAndPostId_whenLoadingExistingPost_thenReturnsPost() {
        ...
        var post = Post.of("게시물 제목", "삼성전자", "게시물 내용", 0L, 0L, 0L, githubUserInfo);

        given(likeDislikeService.doesUserLikePost(unchangeableId, postId)).willReturn(false);
        given(likeDislikeService.doesUserDislikePost(unchangeableId, postId)).willReturn(false);
        ...
        // Then
        assertThat(result)
                .extracting("body", "isLiking")
                .containsExactly("게시물 내용", false);
        then(postRepository).should().findById(postId);
        then(likeDislikeService).should().doesUserLikePost(unchangeableId, postId);
        then(likeDislikeService).should().doesUserDislikePost(unchangeableId, postId);
    ...
    @Test
    void givenNothing_whenLoadingAllSimplePostList_thenReturnsAllSimplePostList() {
        // Given
        var githubUserInfo = GithubUserInfo.of("123456", "test-id", "test-name", "test@email.com", 0.1);
        List<Post> posts = List.of(Post.of("게시물 제목", "삼성전자", "게시물 내용", 0L, 1L, 0L, githubUserInfo));
        ...
        assertThat(result)
                .hasSize(1)
                .extracting("title", "stockName", "likesCount")
                .containsExactly(tuple("게시물 제목", "삼성전자", 1L));
    ...
    @Test
    void givenUnchangeableId_whenLoadingAllMySimplePostList_thenReturnsAllMySimplePostList() {
        // Given
        String unchangeableId = "123456";
        var githubUserInfo = GithubUserInfo.of("123456", "test-id", "test-name", "test@email.com", 0.1);
        List<Post> posts = List.of(Post.of("게시물 제목", "SK하이닉스", "게시물 내용", 0L, 0L, 0L, githubUserInfo));
        ...
        assertThat(result)
                .hasSize(1)
                .extracting("title", "stockName", "likesCount")
                .containsExactly(tuple("게시물 제목", "SK하이닉스", 0L));
    ...
    void givenNonExistentPost_whenUpserting_thenCreatesPost() {
        // Given
        var githubUserInfo = GithubUserInfo.of("123456", "test-id", "test-name", "test@email.com", 0.1);
        var postDto = PostDto.of("게시물 제목", "삼성전자", "게시물 내용", "test-name", "123456");
        var post = Post.of(1L, "게시물 제목", "삼성전자", "게시물 내용", 0L, 0L, 0L, githubUserInfo);
    ...
    void givenUnchangeableIdAndPostId_whenTogglingLikes_thenUpdatesLikesCount() {
        ...
        given(likeRepository.findByGithubUserInfoUnchangeableIdAndPostPostId(unchangeableId, postId)).willReturn(Optional.empty());
        given(dislikeRepository.findByGithubUserInfoUnchangeableIdAndPostPostId(unchangeableId, postId)).willReturn(Optional.empty());
        given(postRepository.save(post)).willReturn(post);


        // When
        PostDto result = sut.toggleLike(unchangeableId, postId);
    ...
    void givenUnchangeableIdAndPostId_whenTogglingDislikes_thenUpdatesDislikesCount() {
        ...
        // When
        PostDto result = sut.toggleDislike(unchangeableId, postId);
