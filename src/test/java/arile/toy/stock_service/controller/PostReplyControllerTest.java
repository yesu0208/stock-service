...
class PostReplyControllerTest {
    ...
    @Test
    void givenAuthenticatedUser_whenRequesting_thenShowsEmptyPost() throws Exception {
...
//                .andExpect(model().attributeExists("replies"))
    ...
    void givenAuthenticatedUserAndPostId_whenToggleLikes_thenPostLikesChanges() throws Exception {
        ...
        var githubUser = new GithubUser("12345", "test-id", "test-name", "test@eamil.com");
        Long postId = 1L;
        given(postService.toggleLike(githubUser.unchangeableId(), postId)).willReturn(
                PostDto.of(1L, "post", "삼성전자", "삼성전자", 0L,
                                1L, 1L, LocalDateTime.now(), LocalDateTime.now(),
                        "test-name", "12345", true, false));
        ...
        then(postService).should().toggleLike(githubUser.unchangeableId(), postId);
    }
    ...
    void givenAuthenticatedUserAndPostId_whenToggleDislikes_thenPostDislikesChanges() throws Exception {
        ...
        var githubUser = new GithubUser("12345", "test-id", "test-name", "test@eamil.com");
        Long postId = 1L;
        given(postService.toggleDislike(githubUser.unchangeableId(), postId)).willReturn(
                PostDto.of(1L, "post", "삼성전자", "삼성전자", 0L,
        ...
        mvc.perform(post("/post/dislikes/{postId}", postId)
                ...
                .andExpect(jsonPath("$.isDisliking").value(false))
                .andExpect(jsonPath("$.dislikesCount").value(1L));
        then(postService).should().toggleDislike(githubUser.unchangeableId(), postId);
    }
